...
public class ControllerManagerScript : MonoBehaviour
{
   ...
   //     laser.SetPositions(initLaserPositions);
    ...
    void Update()
    {
        Quaternion ori = device.Orientation;
        gameObject.transform.localRotation = ori;

        Vector3 v = device.Orientation * Vector3.forward;

        ShootLaserFromTargetPosition(transform.position, v, 200f);
      //  laser.enabled = true;

        if (device.GetButtonUp (GvrControllerButton.TouchPadTouch)) //if the user touches the touchpad it will count as a button press check for other uses and buttons
        {
