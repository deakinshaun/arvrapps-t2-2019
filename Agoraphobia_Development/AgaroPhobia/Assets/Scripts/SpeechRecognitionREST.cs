...
private static bool TrustCertificate(object sender, X509Certificate x509Certificate, X509Chain x509Chain, SslPolicyErrors sslPolicyErrors)
    ...
    public void SpeechToText(byte[] wavData)
    {
       ...
       // TextM.text = "Responsefrom service: " + responseString;


        if(responseString.Contains("Up"))
        {
            TextM.text = "Going Up";
            GameObject.FindWithTag("ElevatorFloor").GetComponent<ElevatorScript>().goingUp();
        }
        if(responseString.Contains("Down"))
        {
            TextM.text = "Going Down";
            GameObject.FindWithTag("ElevatorFloor").GetComponent<ElevatorScript>().goingDown();
        }
...
private IEnumerator recordAudio()
    ...
    public void Trigger()
    {
       ...
       // Authentication();
        ...
        //after authenitcation checked and working, uncomment record audio 
