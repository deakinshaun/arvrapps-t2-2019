...
{
    private string subscriptionKey = "9e444bef5dd14512911e11d714e826e7";
    ...
    void OnVoiceCommandResponse(string command)
    {
        ...
        {
            GameObject g = Instantiate(sphere, new Vector3(UnityEngine.Random.Range(-5.0f, 5.0f), 3.0f,
                UnityEngine.Random.Range(-5.0f, 5.0f)), Quaternion.identity);
            g.GetComponent<MeshRenderer>().material.color = UnityEngine.Random.ColorHSV(0, 1, 0.5f, 1, 0.5f, 1);
        }
    }
    private IEnumerator recordAudio()
    {
        statusText.text = "Waiting voice command";
    ...
    void Update()
    {
        //Debug.Log("Unity acceleration magnitude " + Input.acceleration.magnitude);
        if ((Input.acceleration.magnitude >= shakeDetectionThreshold) && (Time.time >= timeFromLastShake + shakeDetectionInterval))
        {
            timeFromLastShake = Time.time;
            Trigger();
        }
