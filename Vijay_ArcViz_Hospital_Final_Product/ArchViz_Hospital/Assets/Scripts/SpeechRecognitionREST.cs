...
public class SpeechRecognitionREST : MonoBehaviour
{
    ...
    public GameObject executeAlarm;
    ...
    public void SpeechToText(byte[] wavData)
    {
        ...
        if (responseString.Contains("Help"))
        {
            Debug.Log("Key Detected");
            executeAlarm.GetComponent<AlarmLight>().alarmOn = true;
    ...
    {
        if(executeAlarm.GetComponent<AlarmLight>().alarmOn == true)
        {
            executeAlarm.GetComponent<AlarmLight>().alarmOn = false;
            sirenAudio.GetComponent<AudioSource>().Stop();
        }
    }
